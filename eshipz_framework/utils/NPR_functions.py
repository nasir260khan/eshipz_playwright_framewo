...
def search_shipment_by_customer_refexce(page, base_url, custref,awb_text):
    page.goto(f"{base_url}/v2/exceptions/npr/actionRequired")
    page.wait_for_load_state("networkidle")
...
def update_tracking_exception(page):
    ...
    page.get_by_role("option", name="PickupException").click()
...
def open_npr_action(page):
    page.locator("td > div > div:nth-child(2)").click()
    page.get_by_role("button", name="NPR", exact=True).click()
    page.locator("#npr-action").click()

#---------------- reshcheduleing action for npr-----------------------------
def fill_npr_reschedule_date(page):
    today = datetime.now()
...
def fill_npr_edit_and_reschedule(page):
...
def CANCELFUNC(page):
    ...
    assert "Pickup is successfully cancelled" in popup_text

    print("✅ NPR Cancel success validated")
# ------------------------------------------------------------------------------
...
def confirm_npr_action(page, awb_text=None,action_type=None):
    if action_type in ["RESCHEDULE", "EDIT_AND_RESCHEDULE"]:
        page.get_by_role("button", name="Confirm").click()
        time.sleep(3)
...
def search_awb_in_npr_action_requested(page, base_url, awb_text):
    page.goto(f"{base_url}/v2/exceptions/npr/actionRequested")
    page.wait_for_load_state("networkidle")
...
def update_carrier_tracking_status(page, awb_text):
    ...
    page.get_by_text("ExceptionPickupException").click()
    page.get_by_role("button", name="Tracking").click()
    page.get_by_role("button", name="Update Tracking").click()
    page.get_by_role("combobox", name="Tracking Status *").click()
    page.get_by_role("option", name="PickedUp").click()
    page.get_by_role("combobox", name="Tracking Sub Status *").click()
    page.get_by_role("option", name="PickedUp").click()
    page.get_by_role("button", name="Update", exact=True).click()
    ...
    if row.count() > 0 and row.is_visible():
        print(f"✅ Shipment has moved from NPR Action Requested page to picked tab with the shipment AWB number: {awb_text}")
        row_text = row.inner_text()
        assert awb_text in row_text
    else:
        print(f"❌ Shipment not present in NPR Action Requested page : {awb_text}")
